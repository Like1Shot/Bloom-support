# Bloom - App Store Documents

Official documentation and legal documents for **Bloom - Period & Cycle Tracker**.

## Documents

| File | Description |
|------|-------------|
| [PRIVACY_POLICY.md](PRIVACY_POLICY.md) | Privacy Policy for Bloom |
| [TERMS_OF_SERVICE.md](TERMS_OF_SERVICE.md) | Terms of Service |

## URLs for App Store

Use these URLs when submitting to App Store Connect:

| Purpose | URL |
|---------|-----|
| Privacy Policy | `https://github.com/Like1Shot/Bloom-support/main/PRIVACY_POLICY.md` |
| Terms of Service | `https://github.com/Like1Shot/Bloom-support/blob/TERMS_OF_SERVICE.md` |
| Support | `https://github.com/Like1Shot/Bloom-support` |

## Contact

- **GitHub Issues**: https://github.com/Like1Shot/Bloom-support/issues
- **Instagram**: [@bloomapp](https://instagram.com/bloomapp)

## Version History

| Version | Date | Changes |
|---------|------|---------|
| 1.0 | Feb 10, 2026 | Initial release |

---

© 2026 Bloom. All rights reserved.
